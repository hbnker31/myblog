...
export async function updateBlog(blogId: string, data: blogFormSchemaType) {
    const supabase = await createSupabaseServerClient();
...
export async function readBlogContent(id: string) {
    const supabase = await createSupabaseServerClient();
...
export async function updateBlogDetail(blogId: string, data: blogFormSchemaType) {
    ...
    if (result?.error) {
        ...
        const details = await supabase.from("blog_content").update({ content: data.content }).eq("blog_id", blogId);
        revalidatePath(DASHBOARD);
        revalidatePath(`/blog/${blogId}`);
